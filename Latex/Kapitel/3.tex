...
Einer der Hauptbestandteile für die Durchführung von Berechnungen mit \gls{TF} sind Datenflussgraphen, im Original "`dataflow graph"' genannt, welcher aus einer bestimmten Anzahl an Knoten besteht. Dieser wird im Quellcode meist zu Beginn über eine der verfügbaren Client-Schnittstellen (Python oder C++) definiert. \autoref{lst:tf-graph} zeigt den Beispiel-Code für die Definition eines sehr einfachen Datenflussgraphes, bei dem zunächst in Zeile zwei ein Vektor b von der Größe 100 mit Nullen initialisiert wird. Anschließend wird in Zeile drei eine Matrix W mit den Dimensionen 625x100 mit Zufallswerten zwischen minus Eins und Eins belegt und in Zeile vier ein variabler Platzhalter x für die spätere Eingabe definiert. Der eigentliche Graph entsteht in Zeile fünf: Zunächst wird der Input x mit der Matrix W multipliziert und anschließend der Vektor b aufaddiert. Das Ergebnis wird anschließend einer ReLU-Funktion (\autoref{sub:relu}) übergeben. Diese Verkettung kann beliebig lange weitergeführt werden, was im Code in Zeile sechs und in der grafischen Darstellung in \autoref{fig:dataflow} durch den Platzhalter symbolisiert wird.

\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, label=lst:tf-graph, caption={Code-Beispiel zur Definition eines Graphen}]
...
\begin{figure}[htb!]
	...
	\vspace{10pt} 
	\caption{Datenflussgraph zu \autoref{lst:tf-graph}}
	\label{fig:dataflow}
\end{figure}
